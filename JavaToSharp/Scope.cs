...
namespace JavaToSharp
{
    internal class Scope
    {
        ...
        protected virtual void draw2d(Graphics g)
        {
            ...
            {
                g.DrawString(text, font,whiteBrush,x, yt);
                yt += 15;
            }
        }
        ...
        internal virtual void draw(Graphics g)
        {
           ...
           SolidBrush myBrush = new SolidBrush( g.GetNearestColor(Color.White));
            Font o = new Font("SanSerif", 14, FontStyle.Bold);
            int yt = rect.Y+10;
            x += rect.X;
            if (BshowMax)
            {
                if (value != 0)
                    g.DrawString(elm.getUnitText(realMaxV, elm.getScopeUnits(value)), o,myBrush,x, yt);
                else if (showV)
                    g.DrawString(elm.getVoltageText(realMaxV), o,myBrush,x, yt);
                else if (showI)
                    g.DrawString(elm.getCurrentText(realMaxI), o,myBrush,x, yt);
                yt += 15;
            }
            if (BshowMin)
            {
                ...
                if (value != 0)
                    g.DrawString(elm.getUnitText(realMinV, elm.getScopeUnits(value)), o,myBrush,x, ym);
                else if (showV)
                    g.DrawString(elm.getVoltageText(realMinV), o,myBrush ,x, ym);
                else if (showI)
                    g.DrawString(elm.getCurrentText(realMinI),o,myBrush, x, ym);
            }
            if (text != null && rect.Y + rect.Height > yt+5)
            {
                ...
                FontFamily fontFamily = new FontFamily("SansSerif");
                Font font = new Font(fontFamily, 16, FontStyle.Regular, GraphicsUnit.Pixel);
                g.DrawString(text, font,myBrush,x, yt);
                yt += 15;
            }
            FontFamily font_Family = new FontFamily("SansSerif");
            Font q_font = new Font(font_Family, 16, FontStyle.Regular, GraphicsUnit.Pixel);
            if (BshowFreq && freq != 0 && rect.Y + rect.Height > yt+5)
                g.DrawString(elm.getUnitText(freq, "Гц"),q_font,myBrush ,x, yt);
            if (ptr > 5 && !lockScale)
            {
...
//		
            ...
            if (pixels == null)
            {
                ...
                imageSource = new Bitmap(w, h, g);
                //imageSource.Animated = true;
               // imageSource.FullBufferUpdates = true;
               // image = sim.cv.cre(imageSource);
            }
            dpixels = new float[w*h];
            draw_ox = draw_oy = -1;
        }

        internal virtual void handleMenu(EventArgs e, object mi)
        {
