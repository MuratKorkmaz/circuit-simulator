...
public abstract class CircuitElm : Editable
{
    ...
    internal static void initClass(CirSim s)
	{
	    ...
	    showFormat = DecimalFormat.Instance;
	    showFormat.MaximumFractionDigits = 2;
	    shortFormat = DecimalFormat.Instance;
	    shortFormat.MaximumFractionDigits = 1;
    ...
    {
        if (sim.stoppedCheck.Checked || Math.Abs(pos - 0) < double.Epsilon || !sim.dotsCheckItem.State)
            return;
        int dxp = pb.X - pa.X;
        int dyp = pb.Y - pa.Y;
    ...
    internal static string getUnitText(double v, string u)
    {
        ...
        if (va < 1e-14)
            return "0 " + u;
        if (va < 1e-9)
            return showFormat.format(v*1e12) + " п" + u;
        if (va < 1e-6)
            return showFormat.format(v*1e9) + " н" + u;
        if (va < 1e-3)
            return showFormat.format(v*1e6) + " " + CirSim.muString + u;
        if (va < 1)
            return showFormat.format(v*1e3) + " м" + u;
        if (va < 1e3)
            return showFormat.format(v) + " " + u;
        if (va < 1e6)
            return showFormat.format(v*1e-3) + " к" + u;
        if (va < 1e9)
            return showFormat.format(v*1e-6) + " М" + u;
        return showFormat.format(v*1e-9) + " Г" + u;
    }
    ...
    internal static string getShortUnitText(double v, string u)
    {
        ...
        if (va < 1e-13)
            return null;
        if (va < 1e-9)
            return shortFormat.format(v*1e12) + "п" + u;
        if (va < 1e-6)
            return shortFormat.format(v*1e9) + "н" + u;
        if (va < 1e-3)
            return shortFormat.format(v*1e6) + CirSim.muString + u;
        if (va < 1)
            return shortFormat.format(v*1e3) + "м" + u;
        if (va < 1e3)
            return shortFormat.format(v) + u;
        if (va < 1e6)
            return shortFormat.format(v*1e-3) + "к" + u;
        if (va < 1e9)
            return shortFormat.format(v*1e-6) + "М" + u;
        return shortFormat.format(v*1e-9) + "Г" + u;
    }
...
//	  return;
//	  }
        return !sim.powerCheckItem.State ? Color.Empty : setPowerColor(Power);
    }
