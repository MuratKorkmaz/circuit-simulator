...
{
    internal class VCOElm : ChipElm
    {
        public VCOElm(int xx, int yy) : base(xx, yy)
        {
        }
        public VCOElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) : base(xa, ya, xb, yb, f, st)
        {
        }
        ...
        private const double cResistance = 1e6;
        private int cDir;
        ...
        protected virtual void computeCurrent()
        {
            if (Math.Abs(cResistance - 0) < double.Epsilon)
                return;
