...
//JAVA TO VB & C# CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
        ...
        {
            arr[0] = i_position == 0 ? "СЂРµР»Рµ (РІС‹РєР»)" : i_position == 1 ? "СЂРµР»Рµ (РІРєР»)" : "СЂРµР»Рµ";
            int i;
            int ln = 1;
            for (i = 0; i != poleCount; i++)
                arr[ln++] = "I" + (i+1) + " = " + getCurrentDText(switchCurrent[i]);
            arr[ln++] = "РѕР±РјРѕС‚РєР° I = " + getCurrentDText(coilCurrent);
            arr[ln++] = "РѕР±РјРѕС‚РєР° Vd = " + getVoltageDText(volts[nCoil1] - volts[nCoil2]);
        }
        public override EditInfo getEditInfo(int n)
        {
            if (n == 0)
                return new EditInfo("РРЅРґСѓРєС‚РёРІРЅРѕСЃС‚СЊ (Р“РЅ)", inductance, 0, 0);
            if (n == 1)
                return new EditInfo("РЎРѕРїСЂРѕС‚РёРІР»РµРЅРёРµ РІРєР» (РћРј)", r_on, 0, 0);
            if (n == 2)
                return new EditInfo("РЎРѕРїСЂРѕС‚РёРІР»РµРЅРёРµ РІС‹РєР» (РћРј)", r_off, 0, 0);
            if (n == 3)
                return new EditInfo("РўРѕРє РІРєР» (A)", onCurrent, 0, 0);
            if (n == 4)
                return new EditInfo("РљРѕР»РёС‡РµСЃС‚РІРѕ РєРѕРЅС‚Р°РєС‚РЅС‹С… РіСЂСѓРїРї", poleCount, 1, 4).setDimensionless();
            if (n == 5)
                return new EditInfo("РЎРѕРїСЂРѕС‚РёРІР»РµРЅРёРµ РѕР±РјРѕС‚РєРё (РћРј)", coilR, 0, 0);
            if (n == 6)
            {
                EditInfo ei = new EditInfo("", 0, -1, -1);
                ei.checkbox = new Checkbox("РџРѕРјРµРЅСЏС‚СЊ РЅР°РїСЂР°РІР»РµРЅРёРµ РѕР±РјРѕС‚РєРё", (flags & FLAG_SWAP_COIL) != 0);
                return ei;
            }
            return null;
        }
