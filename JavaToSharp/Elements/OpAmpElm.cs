...
namespace JavaToSharp.Elements
{
    internal class OpAmpElm : CircuitElm
    {
        ...
        public OpAmpElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) : base(xa, ya, xb, yb, f)
        {
            ...
            try
            {
                string sMaxOut = st.nextToken();
                bool isParsedMaxOut = double.TryParse(sMaxOut,out maxOut);
                if (!isParsedMaxOut)
                {
                    throw new Exception("Не удалось привести к типу double"); 
                }
                string sMinOut = st.nextToken();
                bool isParsedMinOut = double.TryParse(sMinOut,out minOut);
                if (!isParsedMinOut)
                {
                    throw new Exception("Не удалось привести к типу double");
                }
                string sGbw = st.nextToken();
                bool isParsedGbw = double.TryParse(sGbw,out gbw);
                if (!isParsedGbw)
                {
                    throw new Exception("Не удалось привести к типу double");
                }
        ...
        internal override void draw(Graphics g)
        {
            ...
            drawThickLine(g, myPen ,in1p[0], in1p[1]);
