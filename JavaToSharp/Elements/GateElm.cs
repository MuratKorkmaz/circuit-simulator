    ...
    internal abstract class GateElm : CircuitElm
    {
        ...
        public GateElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) : base(xa, ya, xb, yb, f)
        {
            ...
            if (!isParsedInputCount)
            {
             ...
             // >2.5
            string sLastOutput = st.nextToken();
        
            
            bool isParsedLastOutput = bool.TryParse(sLastOutput, out lastOutput);
            if (!isParsedLastOutput)
            {
        ...
        internal override void draw(Graphics g)
        {
            ...
            for (i = 0; i != inputCount; i++)
            {
              voltageColor=  setVoltageColor(g, volts[i]);
                ...
                drawThickLine(g, myPen,inPosts[i], inGates[i]);
            }
           voltageColor = setVoltageColor(g, volts[inputCount]);
            ...
            if (linePoints != null)
                for (i = 0; i != linePoints.Length-1; i++)
                    drawThickLine(g, myPen,linePoints[i], linePoints[i+1]);
            if (isInverting)
