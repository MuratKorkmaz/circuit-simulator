...
{
    internal class TappedTransformerElm : CircuitElm
    {
        ...
        private new new readonly double[] curcount;
        ...
        public TappedTransformerElm(int xa, int ya, int xb, int yb, int f, StringTokenizer st) : base(xa, ya, xb, yb, f)
        {
            ...
            try
            {
                string sCurrent2 = st.nextToken();
            bool isParsedCurrent2 = double.TryParse(sCurrent1,out current[2]);
            if(!isParsedCurrent2)
            {
                throw new Exception("Не удалось привести к типу double");
            }
