...
namespace JavaToSharp
{
    internal class LampElm : CircuitElm
    {
        ...
        internal override void draw(Graphics g)
        {
            ...
            myBrush = new SolidBrush(TempColor);
            g.FillEllipse(myBrush,bulb.X-bulbR, bulb.Y-bulbR, bulbR*2, bulbR*2);
            g.GetNearestColor(Color.White);
            drawThickCircle(g, bulb.X, bulb.Y, bulbR);
            voltageColor=	setVoltageColor(g, v1);
            myPen = new Pen(voltageColor);
            drawThickLine(g, myPen,lead1, filament[0]);
            voltageColor=	setVoltageColor(g, v2);
            myPen = new Pen(voltageColor);
            drawThickLine(g, myPen,lead2, filament[1]);
            voltageColor =	setVoltageColor(g, (v1+v2)*.5);
            myPen = new Pen(voltageColor);
            drawThickLine(g, myPen,filament[0], filament[1]);
            updateDotCount();
            if (sim.dragElm != this)
            {
